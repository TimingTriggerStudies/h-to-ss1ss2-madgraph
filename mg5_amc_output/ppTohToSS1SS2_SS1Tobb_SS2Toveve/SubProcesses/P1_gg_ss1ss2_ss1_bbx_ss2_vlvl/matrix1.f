...
C     
C     Process: g g > h > ss1 ss2 WEIGHTED<=2 @1
C     *   Decay: ss1 > b b~ WEIGHTED<=1
C     *   Decay: ss2 > ve ve~ WEIGHTED<=1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     Needed for v4 models
      ...
      DOUBLE PRECISION FK_MDL_WSS2
      DOUBLE PRECISION FK_MDL_WSS1
...
C     ----------
      IF (FIRST) THEN
        FIRST=.FALSE.
        FK_MDL_WSS1 = SIGN(MAX(ABS(MDL_WSS1), ABS(MDL_MSS1
     $   *SMALL_WIDTH_TREATMENT)), MDL_WSS1)
        FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MB*SMALL_WIDTH_TREATMENT)
     $   ), ZERO)
        FK_MDL_WSS2 = SIGN(MAX(ABS(MDL_WSS2), ABS(MDL_MSS2
     $   *SMALL_WIDTH_TREATMENT)), MDL_WSS2)
     ...
     $   , ZERO)
      ...
      CALL FFS1_3(W(1,4),W(1,3),GC_12,MDL_MSS1, FK_MDL_WSS1,W(1,5))
      CALL OXXXXX(P(0,5),ZERO,NHEL(5),+1*IC(5),W(1,4))
      CALL IXXXXX(P(0,6),ZERO,NHEL(6),-1*IC(6),W(1,3))
      CALL FFS1_3(W(1,3),W(1,4),GC_13,MDL_MSS2, FK_MDL_WSS2,W(1,6))
      CALL VVS2_3(W(1,1),W(1,2),GC_8,MDL_MH, FK_MDL_WH,W(1,3))
C     Amplitude(s) for diagram number 1
      CALL SSS1_0(W(1,3),W(1,5),W(1,6),GC_11,AMP(1))
